...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.COCOOP_PROTOTYPE.N_CTX
        ctx_init = cfg.TRAINER.COCOOP_PROTOTYPE.CTX_INIT
        ...
        if cfg.TRAINER.COCOOP_PROTOTYPE.PREC == "fp16":
            self.meta_net.half()
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        # print("img before norm : ",image_features)
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        # print("img after norm : ",image_features)
        ...
        if label is not None:
            ...
            return image_features, task_loss
        
        return logits

@TRAINER_REGISTRY.register()
class CoCoOp_Prototype(TrainerX):
    ...
    def check_cfg(self, cfg):
        assert cfg.TRAINER.COCOOP_PROTOTYPE.PREC in ["fp16", "fp32", "amp"]
    
    def build_data_loader(self):
        cfg = self.cfg
        dataset = build_dataset(cfg)
        print("Dataset Name:", cfg.DATASET.NAME)
        # print("Train data sample:", dataset.train_x[:5])
        print()
    ...
    def compute_prototype_loss(self, image_features, labels, alpha=0.7):
        ...
        # print("image_features stats - min:", image_features.min(), "max:", image_features.max())
        # print("prototypes stats - min:", prototypes.min(), "max:", prototypes.max())
        ...
        if labels.ndim > 1 and labels.size(-1) > 1:  # One-hot encoded 상태
            ...
            # print("Converted labels.shape:", labels.shape)
        ...
        # ce_loss = F.cross_entropy(similarity, labels) # 다른 클래스 프로토타입과의 관계 -> Batch Size 1은 불가(CoCoOp Default)
        # bs = image_features.size(0) # 그럼 Pseudo Batch를 만들어서? -> 의미 없는 행동, 위 정보로 충분히 똑같은 기능 가능
        # pseudo_batch = torch.cat([image_features, self.visual_prototypes], dim=0)
        # pseudo_similarity = torch.matmul(pseudo_batch, pseudo_batch.t())
        # negative_loss = pseudo_similarity[:bs, bs:].mean()
        
        # mask = torch.ones_like(similarity, dtype=torch.bool)
        # mask[torch.arange(similarity.size(0)), labels] = False
        # negative_loss = similarity[mask].mean() # 모든 나머지 클래스와의 유사도를 단일 평균값으로 단순화 -> 클래스마다 개별적인 관계를 학습하지 못함
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.COCOOP_PROTOTYPE.PREC == "fp32" or cfg.TRAINER.COCOOP_PROTOTYPE.PREC == "amp":
            # CLIP's default precision is fp16
            clip_model.float()
            
        # Build Visual Prototypes & Criticism
        ...
        self.scaler = GradScaler() if cfg.TRAINER.COCOOP_PROTOTYPE.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.COCOOP_PROTOTYPE.PREC
        if prec == "amp":
            with autocast():
                features, task_loss = model(image, label)
        else:
            features, task_loss = model(image, label)
