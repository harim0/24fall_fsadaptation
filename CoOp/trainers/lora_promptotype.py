...
import sys
sys.path.append('/home/harim/CLIP_LoRA')
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, superclass_mapping, clip_model):
        ...
        n_ctx = cfg.TRAINER.LORA_PROMPTOTYPE.N_CTX
        ctx_init = cfg.TRAINER.LORA_PROMPTOTYPE.CTX_INIT
        ctx_mid = cfg.TRAINER.LORA_PROMPTOTYPE.CTX_MID
        ...
        if cfg.TRAINER.LORA_PROMPTOTYPE.PREC == "fp16":
            self.meta_net.half()
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, superclass_names, clip_model):
        super().__init__()
        self.lora_encoder = cfg.TRAINER.LORA_PROMPTOTYPE.ENCODER
        ...
        self.dtype = clip_model.dtype
        self.template = cfg.TRAINER.LORA_PROMPTOTYPE.CTX_INIT
    ...
    def forward(self, image, label=None, superclass_label=None):
        ...
        for pts_i, imf_i in zip(prompts, image_features):
            ...
            superclass_logits = logit_scale * imf_i @ self.prompt_learner.superclass_embeddings.weight.t().to(imf_i.dtype)
...
class LoRA_Promptotype(TrainerX):
    def __init__(self, cfg, rank, world_size):
        super().__init__(cfg, rank, world_size)
        assert self.rank is not None, "\t\tRank is not set properly in LoRA_Prototype"
        self.rank = rank
        self.world_size = world_size
        self.device = torch.device(f"cuda:{rank}")
        
    def check_cfg(self, cfg):
        assert cfg.TRAINER.LORA_PROMPTOTYPE.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_visual_protocriticism(self, cfg, clip_model, train_loader):
        ...
        if os.path.exists(key_path) and os.path.exists(value_path):
            ...
            cache_keys = torch.cat(cache_keys, dim=0) # (num_samples, feature_dim) torch.Size([1000, 512]), self.dtype
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.LORA_PROMPTOTYPE.PREC == "fp32" or cfg.TRAINER.LORA_PROMPTOTYPE.PREC == "amp":
        ...
        for name, param in self.model.named_parameters():
            if 'lora_' not in name and 'prompt_learner' not in name:
        ...
        self.optim_prompt_learner = build_optimizer(self.model.prompt_learner, cfg.OPTIM_PROMPTLEARNER)  
        self.optim_lora_layers = build_optimizer(
            [p for layer in self.model.list_lora_layers for p in layer.parameters()],
            cfg.OPTIM_LORA
        )      
        # self.optim_lora_layers = build_optimizer(self.model.list_lora_layers, cfg.OPTIM)        
        
        # self.sched = build_lr_scheduler(self.optim, cfg.OPTIM)
        
        self.sched_prompt_learner = build_lr_scheduler(self.optim_prompt_learner, cfg.OPTIM_PROMPTLEARNER)
        self.sched_lora_layers = build_lr_scheduler(self.optim_lora_layers, cfg.OPTIM_LORA)
        
        self.register_model("prompt_learner", self.model.prompt_learner, self.optim_prompt_learner, self.sched_prompt_learner)
        self.model.list_lora_layers = nn.ModuleList(self.model.list_lora_layers)
        self.register_model("LoRA_layers", self.model.list_lora_layers, self.optim_lora_layers, self.sched_lora_layers)

        self.scaler = GradScaler() if cfg.TRAINER.LORA_PROMPTOTYPE.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.LORA_PROMPTOTYPE.PREC
        if prec == "amp":
            with autocast():
        ...
        if prec == "amp":
            ...
            # scaler.step(optim)
            # scaler.update()
            # Prompt Learner Optimizer 업데이트
            self.optim_prompt_learner.zero_grad()
            self.scaler.scale(loss).backward(retain_graph=True)
            self.scaler.step(self.optim_prompt_learner)

            # LoRA Optimizer 업데이트
            self.optim_lora_layers.zero_grad()
            self.scaler.scale(loss).backward()
            self.scaler.step(self.optim_lora_layers)
            ...
            self.optim_prompt_learner.step()

            # LoRA Optimizer 업데이트
            self.optim_lora_layers.zero_grad()
            loss.backward()
            self.optim_lora_layers.step()
